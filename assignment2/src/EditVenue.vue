            ...
            <v-flex xs12 sm8 md4>
              <v-card class="elevation-12">
                <v-toolbar dark color="primary">
                  <v-toolbar-title>Edit Venue</v-toolbar-title>
                  <v-spacer></v-spacer>
                </v-toolbar>
                <v-card-text>
                ...
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn color="primary" :disabled="!valid" v-on:click="editVenue()"> Edit </v-btn>
                </v-card-actions>
              </v-card>
            </v-flex>
          </v-layout>
...
<script>
  export  default {
    ...
    methods: {
      ...
      getVenue: function () {
        this . $http . get ( 'http://localhost:4941/api/v1/venues/' + this.venueId)
          .then(function(response) {
            this.venueInfo = response.data;
            this.venueName = response.data['venueName'];
            this.categoryId = response.data['category']['categoryId'],
      ...
      editVenue: function () {
        ...
        this . $http . patch ( 'http://localhost:4941/api/v1/venues/' + this.venueId, {
